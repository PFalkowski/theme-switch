...
| `group-label`, `label-light`, `label-auto`, `label-dark` attributes | Localize the control: `group-label` overrides the radiogroup's own `aria-label` (default `"Theme"`); each `label-*` overrides that option's `aria-label`, and for `label-auto` also its visible text (Light/Dark stay icon-only by design — the icons are already language-neutral). All read once, on connect. |
